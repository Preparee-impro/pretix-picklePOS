...
class POSDashboardView(EventPermissionRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx["items"] = self.request.event.items.prefetch_related("variations").filter(
            active=True
        )
...
class POSCheckoutView(EventPermissionRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        try:
                ...
                for cart_item in cart:
                    ...
                    if cart_item.get("variation"):
                        variation = ItemVariation.objects.get(
                            id=cart_item["variation"], item=item
                        )

                    price = (
                        variation.price
                        if variation and variation.price is not None
            ...
            return JsonResponse(
                {
                    ...
                    + ("updated" if edit_code else "created")
                    + " and checked in!",
                }
            )
            ...
            return JsonResponse(
                {"success": False, "error": "Item not found"}, status=400
            )
        except Order.DoesNotExist:
            return JsonResponse(
                {"success": False, "error": "Order to edit not found"}, status=400
            )
...
class POSSearchView(EventPermissionRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        orders = (
            Order.objects.filter(event=request.event)
            .filter(
...
class POSLoadOrderView(EventPermissionRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        paid_sum = sum(
            p.amount
            for p in order.payments.filter(state=OrderPayment.PAYMENT_STATE_CONFIRMED)
        )
        refunded_sum = sum(
            r.amount for r in order.refunds.filter(state=OrderRefund.REFUND_STATE_DONE)
        )
        net_paid = paid_sum - refunded_sum

        # Count how many of each item/variation exist in this order
        positions = order.positions.values("item_id", "variation_id").annotate(
            qty=Count("id")
        )
        ...
        return JsonResponse(
            {
                ...
                "net_paid": str(
                    net_paid
                ),  # Send the actual paid amount to the frontend
...
class POSCancelOrderView(EventPermissionRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            if not order_code:
                return JsonResponse(
                    {"success": False, "error": "No order code provided"}, status=400
                )
            ...
            with transaction.atomic():
                ...
                refunded_sum = sum(
                    r.amount
                    for r in order.refunds.filter(state=OrderRefund.REFUND_STATE_DONE)
                )
                current_paid = paid_sum - refunded_sum
            ...
            return JsonResponse(
                {
                    "success": True,
                    "message": f"Order {order.code} has been successfully canceled.",
                }
            )
